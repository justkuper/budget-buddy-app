...
export default function TransactionsPage() {
  ...
  // Group by calendar day (sort newest first within each group)
        ...
        {sortedDays.length === 0 ? (
          ...
          sortedDays.map(day => (
            <div key={day} style={{marginBottom:20}}>
              <p className="day-label">{dayLabel(day)}</p>
              <div className="card" style={{padding:0, overflow:'hidden'}}>
                {grouped[day].map((tx, i) => (
                  <div
                    key={tx.id}
                    className={`tx-row ${i < grouped[day].length - 1 ? 'tx-row-border' : ''}`}
                    onClick={() => setConfirmDelete(tx.id)}
                    style={{cursor:'pointer'}}
                  >
                    {/* Icon or bank logo */}
                    <div className="tx-icon" style={{background: categories[tx.category]?.color + '25', position:'relative'}}>
                      {tx.logo_url
                        ? <img src={tx.logo_url} alt="" style={{width:28, height:28, borderRadius:6, objectFit:'contain'}} />
                        : categories[tx.category]?.icon}
                      {/* Small bank badge for Plaid-imported transactions */}
                      {tx.plaid_id && (
                        <span style={{
                          position:'absolute', bottom:-3, right:-3,
                          fontSize:'0.6rem', background:'var(--primary)',
                          color:'white', borderRadius:4, padding:'1px 3px',
                    ...
                    <div className="tx-info">
                      ...
                      <div style={{display:'flex', alignItems:'center', gap:5}}>
                        ...
                        {tx.pending && (
                          <span style={{fontSize:'0.68rem', color:'var(--text-muted)', fontWeight:600}}>· pending</span>
                        )}
                      </div>
                    </div>
